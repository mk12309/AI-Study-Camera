  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Container(
        ...
        decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF818CF8), Color(0xFF6366F1)], begin: Alignment.topLeft, end: Alignment.bottomRight)),
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(30)), child: const Icon(Icons.camera_alt, color: Colors.white, size: 80)),
            const SizedBox(height: 24),
            Text("AI Study Camera\nSnap & Learn", textAlign: TextAlign.center, style: GoogleFonts.outfit(color: Colors.white, fontSize: 32, fontWeight: FontWeight.bold, height: 1.2)),
            const SizedBox(height: 16),
            Text("Capture. Learn. Succeed.", style: GoogleFonts.outfit(color: Colors.white70, fontSize: 16)),
          ],
        ),
      ),
    );
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(children: [
        Expanded(child: PageView(controller: _pageController, onPageChanged: (idx) => setState(() => _currentPage = idx), children: [
          _buildOnboardingPage(image: "https://illustrations.popsy.co/purple/studying.svg", title: "Learn Smarter\nNot Harder", desc: "Snap your notes and get summaries, quizzes, flashcards and audio explanations instantly."),
          _buildOnboardingPage(image: "https://illustrations.popsy.co/purple/success.svg", title: "Master Any\nSubject", desc: "Our AI analyzes your material to create personalized study plans just for you."),
        ])),
        Padding(padding: const EdgeInsets.all(24), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          TextButton(onPressed: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const AuthWrapper())), child: Text("Skip", style: GoogleFonts.outfit(color: Colors.grey, fontSize: 16))),
          Row(children: List.generate(2, (idx) => Container(margin: const EdgeInsets.symmetric(horizontal: 4), width: _currentPage == idx ? 24 : 8, height: 8, decoration: BoxDecoration(color: _currentPage == idx ? const Color(0xFF6366F1) : Colors.grey.shade300, borderRadius: BorderRadius.circular(4))))),
          ElevatedButton(onPressed: () { if (_currentPage < 1) { _pageController.nextPage(duration: const Duration(milliseconds: 300), curve: Curves.ease); } else { Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const AuthWrapper())); } }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF6366F1), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12)), child: Text("Next", style: GoogleFonts.outfit(color: Colors.white, fontWeight: FontWeight.bold))),
        ])),
  ...
  Widget _buildOnboardingPage({required String image, required String title, required String desc}) {
    return Padding(padding: const EdgeInsets.all(40), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Text(title, textAlign: TextAlign.center, style: GoogleFonts.outfit(fontSize: 28, fontWeight: FontWeight.bold, height: 1.2)),
      const SizedBox(height: 16),
      Text(desc, textAlign: TextAlign.center, style: GoogleFonts.outfit(color: Colors.grey, fontSize: 16)),
    ]));
  }
}
...
class _AuthWrapperState extends State<AuthWrapper> {
  ...
  @override
  Widget build(BuildContext context) { return isLogin ? LoginScreen(onSignupTap: toggle) : RegisterScreen(onLoginTap: toggle); }
}
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      final response = await http.post(Uri.parse("$baseUrl/api/login"), headers: {"Content-Type": "application/json"}, body: jsonEncode({"username": _userController.text, "password": _passController.text}));
  ...
  Widget build(BuildContext context) {
    return Scaffold(body: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(children: [
      ...
      const SizedBox(height: 8), Text("Sign in to continue learning", style: GoogleFonts.outfit(color: Colors.grey)),
      const SizedBox(height: 48), _buildInput("Username", Icons.person_outline, _userController),
  ...
  Widget _buildInput(String label, IconData icon, TextEditingController controller, {bool obscure = false}) {
    return TextField(controller: controller, obscureText: obscure, decoration: InputDecoration(labelText: label, prefixIcon: Icon(icon, color: const Color(0xFF6366F1)), border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide.none), filled: true, fillColor: Colors.grey.shade100));
  }
}
...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> _register() async {
    setState(() => _isLoading = true);
    try {
      final response = await http.post(Uri.parse("$baseUrl/api/register"), headers: {"Content-Type": "application/json"}, body: jsonEncode({"username": _userController.text, "password": _passController.text}));
      final data = jsonDecode(response.body);
      if (data['status'] == 'success') {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Account created!"), backgroundColor: Colors.green));
        widget.onLoginTap();
      } else { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(data['message'] ?? "Error"), backgroundColor: Colors.red)); }
    } catch (e) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Error: $e"), backgroundColor: Colors.red)); } finally { if (mounted) setState(() => _isLoading = false); }
  }
  @override
  Widget build(BuildContext context) {
    return Scaffold(body: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(children: [
      Text("Create Account", style: GoogleFonts.outfit(fontSize: 32, fontWeight: FontWeight.bold)),
      const SizedBox(height: 48), _buildInput("Username", Icons.person_add_outlined, _userController),
      const SizedBox(height: 20), _buildInput("Password", Icons.lock_outline, _passController, obscure: true),
      const SizedBox(height: 32),
      _isLoading ? const CircularProgressIndicator() : SizedBox(width: double.infinity, height: 56, child: ElevatedButton(onPressed: _register, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF6366F1), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))), child: Text("Sign Up", style: GoogleFonts.outfit(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)))),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        currentIndex: _currentIndex, onTap: (idx) => setState(() => _currentIndex = idx), type: BottomNavigationBarType.fixed, selectedItemColor: const Color(0xFF6366F1), unselectedItemColor: Colors.grey, showUnselectedLabels: true,
        items: const [
      ...
      floatingActionButton: FloatingActionButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CameraViewScreen())), backgroundColor: const Color(0xFF6366F1), shape: const CircleBorder(), child: const Icon(Icons.camera_alt, color: Colors.white)),
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      if (mounted) setState(() { _recentNotes = jsonDecode(notesRes.body)['notes']?.reversed?.take(3)?.toList() ?? []; });
    } catch (e) { print(e); }
  }
  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFFF8FAFC), body: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 24), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Hello, $globalUsername 👋", style: GoogleFonts.outfit(fontSize: 24, fontWeight: FontWeight.bold)), Text("What do you want to learn today?", style: GoogleFonts.outfit(color: Colors.grey))]),
        const CircleAvatar(backgroundColor: Color(0xFFFFD700), child: Icon(Icons.workspace_premium, color: Colors.white)),
      ])),
      const SizedBox(height: 24),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 24), child: Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF818CF8), Color(0xFF6366F1)]), borderRadius: BorderRadius.circular(24)), child: Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Capture Notes", style: GoogleFonts.outfit(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 8), Text("Use camera to scan your notes", style: GoogleFonts.outfit(color: Colors.white70)), const SizedBox(height: 16), ElevatedButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CameraViewScreen())), style: ElevatedButton.styleFrom(backgroundColor: Colors.white, foregroundColor: const Color(0xFF6366F1)), child: const Text("Scan Now"))])),
        const Icon(Icons.camera_alt, color: Colors.white24, size: 80),
      ]))),
      const SizedBox(height: 32),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 24), child: GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: 3, crossAxisSpacing: 16, mainAxisSpacing: 16, children: [
        _buildActionCard(Icons.folder, "My Notes", const Color(0xFFFFF7ED), const Color(0xFFF97316), () => DefaultTabController.of(context).animateTo(1)),
      ...
      const SizedBox(height: 32),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 24), child: Text("Recent Activity", style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold))),
      const SizedBox(height: 16),
      ListView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.symmetric(horizontal: 24), itemCount: _recentNotes.length, itemBuilder: (context, idx) {
        final note = _recentNotes[idx];
        return InkWell(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ExtractedTextScreen(note: note))), child: Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16)), child: Row(children: [
  ...
  Widget _buildActionCard(IconData icon, String label, Color bg, Color iconColor, VoidCallback onTap) {
    return InkWell(onTap: onTap, child: Container(decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(20)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, color: iconColor), const SizedBox(height: 8), Text(label, style: GoogleFonts.outfit(fontSize: 12, fontWeight: FontWeight.w600, color: iconColor))])));
  }
}
...
class _CameraViewScreenState extends State<CameraViewScreen> {
  ...
  void initState() {
    ...
    if (cameras.isNotEmpty) {
      ...
      _controller!.initialize().then((_) { if (!mounted) return; setState(() => _isReady = true); });
    }
  }
  @override
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Colors.black, body: Stack(children: [
      if (_isReady) Positioned.fill(child: CameraPreview(_controller!)) else Center(child: Text("Camera not available", style: TextStyle(color: Colors.white70))),
      ...
      Positioned(top: 50, right: 20, child: Row(children: [
        IconButton(icon: Icon(_isFlashOn ? Icons.flash_on : Icons.flash_off, color: Colors.white), onPressed: () {
          setState(() => _isFlashOn = !_isFlashOn);
          _controller?.setFlashMode(_isFlashOn ? FlashMode.torch : FlashMode.off);
        }),
        const SizedBox(width: 10), Text("HD", style: GoogleFonts.outfit(color: Colors.white, fontWeight: FontWeight.bold))
      ])),
      Align(alignment: Alignment.bottomCenter, child: Container(height: 200, padding: const EdgeInsets.only(bottom: 40), child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Text("GALLERY", style: GoogleFonts.outfit(color: Colors.grey, fontSize: 12)), const SizedBox(width: 24),
          Text("PHOTO", style: GoogleFonts.outfit(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)), const SizedBox(width: 24),
          Text("DOCUMENT", style: GoogleFonts.outfit(color: Colors.grey, fontSize: 12)),
        ]),
        const SizedBox(height: 24),
        Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
          IconButton(icon: const Icon(Icons.photo_library, color: Colors.white), onPressed: _pickGallery),
          GestureDetector(onTap: _capture, child: Container(width: 80, height: 80, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 4)), child: Center(child: Container(width: 60, height: 60, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle))))),
...
class _ProcessingScreenState extends State<ProcessingScreen> {
  ...
  Future<void> _start() async {
    for (int i = 0; i <= 7; i++) { await Future.delayed(const Duration(milliseconds: 200)); if (mounted) setState(() => _progress = i / 10); }
    try {
      ...
      if (data['status'] == 'success') { Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => ExtractedTextScreen(note: data))); }
    } catch (e) { Navigator.pop(context); }
  }
  @override
  Widget build(BuildContext context) {
    return Scaffold(body: Center(child: Padding(padding: const EdgeInsets.all(40), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Icon(Icons.auto_awesome, color: Color(0xFF6366F1), size: 100),
      const SizedBox(height: 32), Text("Processing Your Notes...", style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.bold)),
      const SizedBox(height: 8), Text("Extracting text and understanding content using AI.", textAlign: TextAlign.center, style: GoogleFonts.outfit(color: Colors.grey)),
      const SizedBox(height: 48), LinearProgressIndicator(value: _progress, backgroundColor: Colors.grey.shade200, color: const Color(0xFF6366F1), minHeight: 8),
      const SizedBox(height: 16), Text("${(_progress * 100).toInt()}%", style: const TextStyle(fontWeight: FontWeight.bold)),
    ]))));
  }
}
...
class ExtractedTextScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(leading: IconButton(icon: const Icon(Icons.chevron_left), onPressed: () => Navigator.pop(context)), title: Text("Extracted Text", style: GoogleFonts.outfit())), body: Padding(padding: const EdgeInsets.all(24), child: Column(children: [
      Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(10)), child: Row(children: [const Icon(Icons.check_circle, color: Colors.green, size: 20), const SizedBox(width: 8), Text("Text extracted successfully!", style: TextStyle(color: Colors.green.shade700, fontSize: 12))])),
      const SizedBox(height: 24), Expanded(child: Container(width: double.infinity, padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16)), child: SingleChildScrollView(child: Text(note['extracted_text'] ?? "")))),
      const SizedBox(height: 24), Row(children: [Expanded(child: OutlinedButton(onPressed: () {}, child: const Text("Edit Text"))), const SizedBox(width: 16), Expanded(child: ElevatedButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SummaryScreen(note: note))), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF6366F1)), child: const Text("Next", style: TextStyle(color: Colors.white))))])
    ])));
  }
}
...
class SummaryScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(leading: IconButton(icon: const Icon(Icons.chevron_left), onPressed: () => Navigator.pop(context)), title: Text("Summary", style: GoogleFonts.outfit())), body: Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text("AI Summary", style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: const Color(0xFF6366F1))),
      const SizedBox(height: 16), Expanded(child: Container(width: double.infinity, padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: const Color(0xFFF0FDF4), borderRadius: BorderRadius.circular(20)), child: SingleChildScrollView(child: Text(note['ai_summary'] ?? "", style: const TextStyle(height: 1.5))))),
      const SizedBox(height: 24), Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [_actionBtn(Icons.copy, "Copy"), _actionBtn(Icons.share, "Share"), _actionBtn(Icons.volume_up, "Listen")]),
      const SizedBox(height: 32), SizedBox(width: double.infinity, height: 56, child: ElevatedButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => QuizScreen(note: note))), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF6366F1)), child: const Text("Take Quiz", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))))
    ])));
  }
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(leading: IconButton(icon: const Icon(Icons.chevron_left), onPressed: () => Navigator.pop(context)), title: Text("Quiz (MCQs)", style: GoogleFonts.outfit())), body: Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text("Question ${_currentQ + 1} of ${quizzes.length}", style: const TextStyle(color: Color(0xFF6366F1), fontWeight: FontWeight.bold)),
      const SizedBox(height: 16), Text(quiz['question'], style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold)),
      const SizedBox(height: 32), ...(quiz['options'] as List).asMap().entries.map((e) => Container(margin: const EdgeInsets.only(bottom: 12), decoration: BoxDecoration(color: _selected == e.key ? const Color(0xFFF5F3FF) : Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: _selected == e.key ? const Color(0xFF6366F1) : Colors.grey.shade200)), child: RadioListTile<int>(title: Text(e.value), value: e.key, groupValue: _selected, activeColor: const Color(0xFF6366F1), onChanged: (v) => setState(() => _selected = v)))),
      const Spacer(), SizedBox(width: double.infinity, height: 56, child: ElevatedButton(onPressed: () { if (_currentQ < quizzes.length - 1) { setState(() { _currentQ++; _selected = null; }); } else { Navigator.push(context, MaterialPageRoute(builder: (_) => FlashcardsScreen(note: widget.note))); } }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF6366F1)), child: Text(_currentQ < quizzes.length - 1 ? "Next Question" : "Finish Quiz", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))))
    ])));
  }
}
...
class _FlashcardsScreenState extends State<FlashcardsScreen> {
  int _current = 0; bool _revealed = false;
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(leading: IconButton(icon: const Icon(Icons.chevron_left), onPressed: () => Navigator.pop(context)), title: Text("Flashcards", style: GoogleFonts.outfit())), body: Padding(padding: const EdgeInsets.all(24), child: Column(children: [
      Align(alignment: Alignment.centerRight, child: Text("${_current + 1} / ${cards.length}", style: const TextStyle(color: Colors.grey))),
      const SizedBox(height: 24), Expanded(child: GestureDetector(onTap: () => setState(() => _revealed = !_revealed), child: Container(width: double.infinity, padding: const EdgeInsets.all(40), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 10)]), child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text(_revealed ? "Answer" : "Question", style: const TextStyle(color: Color(0xFF6366F1), fontWeight: FontWeight.bold)), const SizedBox(height: 20), Text(_revealed ? card['back'] : card['front'], textAlign: TextAlign.center, style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.bold))]))))),
      const SizedBox(height: 48), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [_iconBtn(Icons.shuffle, "Shuffle"), Row(children: [IconButton(icon: const Icon(Icons.chevron_left), onPressed: _current > 0 ? () => setState(() { _current--; _revealed = false; }) : null), IconButton(icon: const Icon(Icons.chevron_right), onPressed: _current < cards.length - 1 ? () => setState(() { _current++; _revealed = false; }) : null)]), _iconBtn(Icons.save_outlined, "Save")]),
      const SizedBox(height: 48), SizedBox(width: double.infinity, height: 56, child: ElevatedButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => AudioExplanationScreen(note: widget.note))), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF6366F1)), child: const Text("Listen to Audio", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))))
    ])));
  }
...
class AudioExplanationScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(leading: IconButton(icon: const Icon(Icons.chevron_left), onPressed: () => Navigator.pop(context)), title: Text("Audio Explanation", style: GoogleFonts.outfit())), body: Padding(padding: const EdgeInsets.all(40), child: Column(children: [
      const Spacer(), Container(padding: const EdgeInsets.all(40), decoration: const BoxDecoration(color: Color(0xFFF5F3FF), shape: BoxShape.circle), child: const Icon(Icons.headphones, color: Color(0xFF6366F1), size: 100)),
      const SizedBox(height: 48), Text(note['filename'] ?? "Study Audio", style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.bold)),
      const SizedBox(height: 8), const Text("AI Generated Explanation", style: TextStyle(color: Colors.grey)),
      const Spacer(), Slider(value: 0.3, onChanged: (v) {}, activeColor: const Color(0xFF6366F1), inactiveColor: Colors.grey.shade200),
      ...
      const SizedBox(height: 32), Row(mainAxisAlignment: MainAxisAlignment.center, children: [IconButton(icon: const Icon(Icons.skip_previous, size: 32), onPressed: () {}), const SizedBox(width: 24), Container(padding: const EdgeInsets.all(16), decoration: const BoxDecoration(color: Color(0xFF6366F1), shape: BoxShape.circle), child: const Icon(Icons.play_arrow, color: Colors.white, size: 48)), const SizedBox(width: 24), IconButton(icon: const Icon(Icons.skip_next, size: 32), onPressed: () {})]),
      const Spacer(),
    ])));
  }
}
...
class PlaceholderScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { return Scaffold(body: Center(child: Text("$name Screen Coming Soon", style: GoogleFonts.outfit(fontSize: 20)))); }
}
